...
import { getDatabase, ref, get, child, set, push,onChildAdded  } from "https://www.gstatic.com/firebasejs/11.2.0/firebase-database.js";
...
const db = getDatabase(app); // تهيئة قاعدة البيانات
      ...
      if (snapshot.exists()) {
        ...
        randomProducts.forEach((product) => {
          ...
          productCard.innerHTML = `
              ...
              <a href="detailsHair.html?id=${product.key}" class="more-details">Read More</a>
          `;
  
          productContainer.appendChild(productCard);
        });
      } else {
        console.log("No data available in Hair Care");
      }
    } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (productId) {
        try {
            ...
            const dbRef = ref(db);
            const snapshot = await get(child(dbRef, `hairCare/${productId}`));
