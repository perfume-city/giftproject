...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (productId) {
        try {
            ...
            const dbRef = ref(db);
            const snapshot = await get(child(dbRef, `bodyCare/${productId}`));
      ...
      if (snapshot.exists()) {
        ...
        randomProducts.forEach((product) => {
          ...
          productCard.innerHTML = `
              ...
              <a href="detailsBody.html?id=${product.key}" class="more-details">Read More</a>
          `;
  
          productContainer.appendChild(productCard);
        });
      } else {
        console.log("No data available in Body care");
      }
    } catch (error) {
