...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (productId) {
        try {
            ...
            const dbRef = ref(db);
            const snapshot = await get(child(dbRef, `perfumWomen/${productId}`));
      ...
      if (snapshot.exists()) {
        ...
        randomProducts.forEach((product) => {
          ...
          productCard.innerHTML = `
              ...
              <a href="detailsWomen.html?id=${product.key}" class="more-details">Read More</a>
          `;
  
          productContainer.appendChild(productCard);
        });
      } else {
        console.log("No data available in perfumeWomen");
      }
    } catch (error) {
