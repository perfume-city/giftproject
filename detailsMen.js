...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (productId) {
        try {
            ...
            const dbRef = ref(db);
            const snapshot = await get(child(dbRef, `perfumMen/${productId}`));
...
async function getPerfumeMenItems() {
    const dbRef = ref(db);
    try {
      const snapshot = await get(child(dbRef, "perfumMen"));
      if (snapshot.exists()) {
        ...
        randomProducts.forEach((product) => {
          ...
          productCard.innerHTML = `
              ...
              <a href="detailsMen.html?id=${product.key}" class="more-details">Read More</a>
          `;
